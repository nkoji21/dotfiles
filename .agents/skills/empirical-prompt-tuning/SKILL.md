...
1. **ベースライン準備**: 対象プロンプトを確定し、次の 3 つを用意する。
   - **評価シナリオ** 2 〜 3 種（中央値 1 + edge 1 〜 2）。現実に起こりうるタスクで、対象プロンプトを実際に適用する場面を想定する。edge の選び方: (a) 最小入力（前提が揃っていない）、(b) 最大入力（情報過多で優先順位が問われる）、(c) 曖昧な表現（複数解釈が可能）、(d) エラー状態（途中で詰まる想定）から選ぶ。中央値と同じ次元の edge は避ける。
   - **hold-out シナリオ** 1 種。毎イテレーションで計測するが、修正の判断材料には使わない（過適合の早期検出用）。
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     - 成功/失敗: `[critical]` タグの付いた要件が **全て ○** かつ精度 50% 以上のとき成功（○）。critical 全て ○ でも精度 50% 未満なら条件付き成功（△）として収束判定には含めない。それ以外は失敗（×）。
     - 精度（要件チェックリストの達成率 %。○ = 満点、× = 0、部分的 = 0.5 で合算、全項目数で割る）
     - ステップ数（Agent tool の usage メタの `tool_uses` をそのまま使う。Read / Grep も含める、除外しない）
...
6. **再評価**: 新しい subagent で再度 2 → 5 を回す（同一 agent は再利用しない: 前回の改善を学習している）。並列度の調整: 改善が続いている間はシナリオの並列度を増やして信頼性を上げる。頭打ちになったら並列度を下げてコストを節約する。
7. **収束判定**: 目安「連続 2 イテレーションで新規の不明瞭点ゼロ かつ メトリクス改善が閾値以下（「反復の打ち切り基準」節を参照）」で停止。重要度が高いプロンプトは 3 連続にする。
...
（注: 判定規則は「ワークフロー 4. 両面評価 / 指示側の計測」節に一元定義。[critical] は最低 1 つ必須。）
...
レポートが構造に従っていない場合は (1) usage メタが取得できていれば steps/duration のみ記録し precision=0 として扱う、(2) usage メタも無ければそのシナリオを invalid として除外し、次 iter で再試行する。subagent が 2 連続で invalid を返す場合はシナリオ設計を見直す。
...
- **収束（停止）**: 連続 2 回で次を **全て** 満たす:
  - 新規不明瞭点: 0 件
  - 精度の前回比改善: +3 ポイント以下（絶対値、例: 85% → 88%）。シナリオ数が 10 項目未満の場合は ±5% を閾値とする。
  - ステップ数の前回比変動: ±10% 以内
  - duration の前回比変動: ±15% 以内
  - **過適合チェック**: hold-out シナリオの精度が訓練シナリオ平均から 15 ポイント以上乖離している場合は過適合。baseline シナリオ設計に戻って edge を足す。
