...
If a `--path <dir>` argument is provided (e.g. invoked as `git-commit-ja --path /tmp/feat-foo`), all git commands must be run inside that directory by prepending `cd <dir> &&` to every Bash command. This overrides the current working directory.
